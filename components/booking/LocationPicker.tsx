...
export const LocationPicker = ({ value, onChange, placeholder = "Search location...", error }: Props) => {
  ...
  const handleSelectSuggestion = (place: any) => {
    ...
    // Use the full name and sync immediately to prevent race conditions that reopen the dropdown
    setSearch(place.display_name);
    setDebouncedSearch(place.display_name);
    ...
    <div className="relative space-y-3" ref={inputRef}>
      <div className="relative flex items-center group">
        ...
        <input
          ...
          className={`w-full pl-12 pr-12 py-4 bg-gray-50/50 rounded-[24px] outline-none font-bold text-gray-800 text-sm focus:bg-white focus:shadow-[0_8px_30px_rgba(0,0,0,0.04)] focus:outline-none focus:ring-0 transition-all ${
        ...
        {search && !isSearching && (
          <button type="button" onClick={handleClear} className="absolute right-5 text-gray-400 hover:text-gray-600 transition bg-transparent border-none outline-none focus:outline-none">
      ...
      {showSuggestions && suggestions.length > 0 && (
        <div className="absolute top-full left-0 right-0 bg-white/95 backdrop-blur-xl border-none outline-none rounded-[24px] shadow-[0_20px_60px_rgba(0,0,0,0.08)] mt-2 z-[1000] max-h-[300px] overflow-y-auto p-2">
          {suggestions.map((place, index) => {
            ...
            return (
              <button
                ...
                key={index}
                onMouseDown={(e) => {
                  e.preventDefault(); // Prevent input blur, avoids focus switching issues
                  handleSelectSuggestion(place);
                }}
                className="w-full px-4 py-3.5 text-left hover:bg-gray-50/80 outline-none border-none focus:outline-none rounded-[16px] transition flex items-center gap-4 group"
              >
