...
export default function DriverTripDetailPage() {
  ...
  const { request, isLoading: isApiLoading, error: apiError } = useApi({ showSuccess: true });
  ...
  useEffect(() => {
    ...
    socket.on('trip_cancelled', (data) => {
      if (String(data.tripId) === String(tripId) || String(data.trip_id) === String(tripId)) {
        toast.error(t("driver.clientCancelled") || "The client cancelled this trip.");
  ...
  const handleStartTrip = async () => {
    ...
    if (result) {
      toast.success(t("driver.TRIP_STARTED") || "Trip started!");
  ...
  const handleEndTrip = async () => {
    ...
    if (result) {
      toast.success(t("driver.TRIP_COMPLETED") || "Trip completed!");
      router.push(`/${locale}/driver/driver-trips`);
    }
  };
  ...
  if (!trip) return <div className="p-8 text-center">{t("driver.tripNotFound")}</div>;
    ...
    <ProtectedRoute allowedRoles={["driver"]}>
      <div className="max-w-2xl mx-auto py-8 px-4">
        <Button variant="ghost" size="sm" onClick={() => router.back()}>
          <FiArrowLeft className="mr-2" /> {t("driver.back")}
        </Button>

        {/* Status Header */}
        <div className="bg-white rounded-xl shadow-md p-6 mb-6 flex justify-between items-center border-t-4 border-primary">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t("driver.tripDetails")}</h1>
            <p className="text-sm text-gray-500">{t("driver.id")}: {trip.id}</p>
          </div>
          <Badge variant="info" label={t(`common.${trip.status}`)} />
        </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-6">
          <div className="space-y-4">
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase">{t("driver.pickupAddress")}</p>
            ...
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase">{t("driver.destinationAddress")}</p>
              <p className="text-gray-900 font-medium">{trip.destination_address || t("driver.openDestination")}</p>
            </div>
          </div>
        ...
        {trip.status === 'accepted' && (
          <div className="bg-primary/5 border border-primary/20 rounded-xl p-6 mb-6">
            ...
            {apiError && (
              <p className="text-red-500 text-sm mt-3 font-medium flex items-center gap-1.5 ml-1">
        ...
        {trip.status === 'in_progress' && (
          ...
          >
            <FiCheckCircle /> {t("driver.completeTrip")}
          </Button>
        )}
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
            <FiDollarSign /> {t("driver.estimatedEarnings")}
          </h3>
          <div className="flex justify-between items-center">
            <span className="text-gray-600">{t("driver.totalFare")}</span>
