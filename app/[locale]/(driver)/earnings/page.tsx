  ...
  useEffect(() => {
    const fetchEarnings = async () => {
      const result = await request<EarningsData>(() =>
        apiClient.getEarnings(dateRange.startDate, dateRange.endDate)
      )
      if (result) {
        setEarnings(result)
      }
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <Card className="mb-8">
          <div className="flex gap-4 items-center">
            <div>
              <label className="block text-sm font-medium mb-2">{t('common.startDate')}</label>
              <input
                type="date"
            ...
            <div>
              <label className="block text-sm font-medium mb-2">{t('common.endDate')}</label>
              <input
                type="date"
        ...
        {isLoading ? (
          ...
          <>
            ...
            {/* Detailed Breakdown */}
            <Card>
              <h2 className="text-2xl font-bold mb-6">{t('common.breakdown')}</h2>
                  ...
                  <tbody>
                    {earnings.weeklyEarnings.map((amount, idx) => (
                      <tr key={idx} className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-4 font-semibold">{amount.toLocaleString()} XOF</td>
                        <td className="p-4 text-green-600">
                          {idx > 0 && amount > earnings.weeklyEarnings[idx - 1] ? '↑' : '→'} +2.5%
                        </td>
                      </tr>
                    ))}
                  </tbody>
